...
extension Matrix {
    ...
    public func matrixMult(by B: Matrix<Double>) -> Matrix<Double> {
        ...
        var C = Matrix<Double>(rows: A.rows, columns: B.columns)
    ...
    public func double()-> Matrix<Double> {
        let A = self
    ...
    public func matrixMultA(by B: Matrix<Double>) -> Matrix<Double> {
        let A = self.double()
        let ra = A.rows
        let cb = B.columns
        let n = A.columns
        ...
        var ab = [Double](repeating: 0.0, count: ra * cb)
        
        let aa = A.miseAPlat()
        let bb = B.miseAPlat()
        
        vDSP_mmulD(aa, 1, bb, 1, &ab, 1, vDSP_Length(ra), vDSP_Length(cb), vDSP_Length(n))
        
        let ab2D = miseen2D(ab, nrows: ra, ncols: cb)
        
        return ab2D
    }
    
    func miseAPlat ()->(Array<Double>) {
